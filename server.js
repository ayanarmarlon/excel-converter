...
app.post("/upload-sales", upload.single("excelFile"), (req, res) => {
    ...
    // Validate Sales Template

    if (
        sheetData[0][2] !== "SUPPLIER TIN" ||
        sheetData[0][3] !== "REGISTER NAME" ||
        sheetData[0][13] !== "COMPANY TIN" ||
        sheetData[0][15] !== "MONTH"
    ) {

        return res.send(`
            <h2>Invalid Template</h2>
            <p>Please use the official Sales Template - Vat Relief.xlsx file.</p>
            <a href="/sales.html">Go Back</a>
        `);
    ...
    for (let rowIndex = 1; rowIndex < sheetData.length; rowIndex++) {
        ...
        if (rowIndex === 1) {
            ...
            row.slice(0, 17).forEach((value, index) => {
        ...
        // Row 3 and below
            ...
            row.slice(0, 15).forEach((value, index) => {
    ...
    const c2 = sheetData[1][2];
    const p2 = sheetData[1][15];
    ...
    const datFileName = `${c2}S${month}${year}.dat`;
